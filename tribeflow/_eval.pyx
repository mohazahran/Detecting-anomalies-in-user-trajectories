...
def reciprocal_rank(double[:, ::1] Dts, int[:, ::1] HOs, \
    ...
    cdef int[::1] mem = np.zeros(Dts.shape[1], dtype='i4') #mem.shape = (B,). Dts.shape[1] is B (history)
    ...
    for i in xrange(HOs.shape[0]): # for all test instances
        dt = Dts[i, Dts.shape[1] - 1] # getting the time of next item (to be predicted)
        h = HOs[i, 0] #the userid
        for j in xrange(mem.shape[0]): # for B
            mem[j] = HOs[i, 1 + j] #mem will hold a list of the B history obj ids. i.e. mem[0] = id(ob1). i.e. Excluding the last item(to be predicted one) 
        ...
        for z in xrange(Psi_sz.shape[1]): #for envs
            mem_factor[z] = 1.0
            for j in xrange(mem.shape[0]):#for all B (history objs)
               #i.e. multiply all psi[objid1,z]*psi[objid2,z]*..psi[objidB,z]
                mem_factor[z] *= Psi_sz[mem[j], z] # Psi[objId, env z]
        
        mem_factor[z] *= 1.0 / (1 - Psi_sz[mem[mem.shape[0] - 1], z])# 1-Psi_sz[mem[B-1],z] == 1-psi_sz[objIdB(the last one in the history),z]
