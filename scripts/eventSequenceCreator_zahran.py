...
def main():
    ...
    for t_now, h_now, o_now in trace:
        if h_now in last and len(last[h_now]) == mem_size:
            t_prev, o_prev = last[h_now][-1] # latest <object,timestamp> added for h_now
        ...
        append = len(last[h_now]) == 0 or \
                consider_loops or \
                ((not consider_loops) and o_now != last[h_now][-1][1]) #last[h_now][-1][1] the last object of h_now
        
        if append:
            last[h_now].append((t_now, o_now))
            if len(last[h_now]) > mem_size:
                del last[h_now][0] # delete the oldest <obj,timestamp> to leave B pairs. (this is the way of having a moving window of B size)
